...
export class Contract {
  ...
  constructor(witnesses = {}) {
    ...
    this.circuits = {
      ...
      disburseSalaryAdvance: (...args) => {
        const contextOrig = args[0];
        if (!contextOrig || typeof contextOrig !== 'object' || contextOrig.currentQueryContext === undefined) {
          throw new __compactRuntime.CompactError('Circuit disburseSalaryAdvance: invalid CircuitContext argument');
        }
        const context = { ...contextOrig, gasCost: __compactRuntime.emptyRunningCost() };
        const partialProofData = _ppd();
        const result = this._disburseSalaryAdvance(context, partialProofData, ...args.slice(1));
  ...
  initialState(constructorContext, ...args) {
    ...
    state.setOperation('disburseSalaryAdvance', new __compactRuntime.ContractOperation());
...
export function ledger(stateOrChargedState) {
  ...
  return {
    pool_authority_vk: 0n,
    total_advances_disbursed: 0n,
    total_fees_collected: 0n,
    last_advance_nullifier: 0n,
    total_spent: spent,
