...
export class Contract {
    ...
    this.circuits = {
      ...
      createStream: (...args) => {
        ...
        if (!contextOrig || typeof contextOrig !== 'object' || contextOrig.currentQueryContext === undefined) {
          throw new __compactRuntime.CompactError('Circuit createStream: invalid CircuitContext argument');
        }
        const context = { ...contextOrig, gasCost: __compactRuntime.emptyRunningCost() };
        const partialProofData = _ppd();
        const result = this._createStream(context, partialProofData, ...args.slice(1));
      ...
      withdrawSalary: (...args) => {
        const contextOrig = args[0];
        if (!contextOrig || typeof contextOrig !== 'object' || contextOrig.currentQueryContext === undefined) {
          throw new __compactRuntime.CompactError('Circuit withdrawSalary: invalid CircuitContext argument');
        }
        const context = { ...contextOrig, gasCost: __compactRuntime.emptyRunningCost() };
        const partialProofData = _ppd();
        const result = this._withdrawSalary(context, partialProofData, ...args.slice(1));
        partialProofData.output = { value: [], alignment: [] };
        return { result, context, proofData: partialProofData, gasCost: context.gasCost };
      },
  ...
  initialState(constructorContext, ...args) {
    ...
    state.setOperation('createStream', new __compactRuntime.ContractOperation());
    state.setOperation('withdrawSalary', new __compactRuntime.ContractOperation());
